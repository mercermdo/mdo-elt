...
/* ── fetch contacts (single pass; HS now allows 300+ props) ─────────── */
...
/* ── batch insert – swallow PartialFailureError ─────────────────────── */
  ...
  for(let i=0;i<rows.length;i+=size){
    ...
    await table.insert(payload,{
      ...
      if(err.name==='PartialFailureError'){
        console.warn('⚠️  batch had row-level errors; first few rows / reasons:');
        err.errors.slice(0,3).forEach(e=>{
          console.warn(JSON.stringify(e.errors),
                       'row snippet:', JSON.stringify(e.row).slice(0,200));
        });
        return;                    // ← **do not re-throw – continue with next batch**
      }
      throw err;                  // genuine fatal error
    });
  }
}
...
(async()=>{
  try{
    ...
    console.log(`✅ Upserted ${rows.length} contacts into ${process.env.BQ_DATASET}.${process.env.BQ_TABLE}`);
  }catch(e){
